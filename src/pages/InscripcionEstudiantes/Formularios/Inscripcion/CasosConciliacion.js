...
const CasosConciliacion = ({ id }) => {
    ...
    const load = async () => {
        const { data } = await API.get(`conciliacion/solicitud/?conciliador=${id}`, { params })
    ...
    const verDetalle = (id) => {
        sessionStorage.setItem("conciliacion", id)
        ...
        <>
            {casos !== [] && (
                <Card style={{ marginTop: "20px" }}>
                    <Card.Body>
                        <h2>Casos de conciliación del estudiante</h2>
                        {casos.length ? (
                            <Table>
                                <thead>
                                    <tr>
                                ...
                                <tbody>
                                    {casos.map((el, i) => (
                                        <tr>
                                            ...
                                            <th>{el?.d_fechaSolicitud}</th>
                                        </tr>
                                    ))}
                                </tbody>
                            </Table>
                        ) : (
                            <Alert variant="warning">Este estudiante no tiene ningún caso de conciliación asignado.</Alert>
                        )}
                    </Card.Body>
...
export default CasosConciliacion