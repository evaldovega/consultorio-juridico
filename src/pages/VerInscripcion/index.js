  ...
  useEffect(() => {
    if (doc) {
      form.setFieldsValue({
        ...
        r_config_ciudadNacimiento:
          doc.r_usuarios_persona.r_config_ciudadNacimiento,
        ...
        a_fechaExpedicionDocumento: Moment(
          doc.r_usuarios_persona.a_fechaExpedicionDocumento
        ),
  ...
  const save = async (data) => {
    let dataJson = {
      r_usuarios_persona: {
        ...
        a_fechaExpedicionDocumento: Moment(
          data.a_fechaExpedicionDocumento
        ).format("YYYY-MM-DD"),
      ...
      dt_fechaInscripcion: Moment(data.dt_fechaInscripcion).format(
        "YYYY-MM-DD"
      ),
    };
    console.log(JSON.stringify(dataJson));
    setLoading(true);
    API.patch("estudiantes/inscripcion/" + id + "/", dataJson)
      .then(({ data }) => {
        setLoading(false);
        notification["success"]({
          message: "Felicitaciones",
          description: "Estas inscrito a las practicas.",
        });
      })
      .catch((error) => {
    ...
    <Page>
      <div style={{ display: "flex", justifyContent: "space-between" }}>
        ...
        <div>
          ...
          />
          <Popconfirm
            title="Se eliminará la inscripción"
            okText="Eliminar"
            cancelText="Cerrar"
          >
            <Button
      ...
      <Tabs
        type="card"
        className="tabs"
        style={{ marginTop: 100, boxShadow: "2px" }}
      >
